...
from e3nn.o3 import Irreps, Instruction #, wigner_3j
...
class L1TensorProduct(Module):
    def __init__(self, in1_irreps: Irreps, out_irreps=None,
        ...
        # self.iri2_l0o = torch.zeros(self.in2_dim, dtype=torch.bool)
        # self.iri2_l1e = torch.zeros(self.in2_dim, dtype=torch.bool)
        self.iri2_l1o = torch.zeros(self.in2_dim, dtype=torch.bool)
        i = 0
        for mir in self.iri2:
            if mir.ir.l == 0:
                mask = self.iri2_l0e #if mir.ir.p == 1 else self.iri2_l0o
                mask[i:i+mir.mul] = True
            elif mir.ir.l == 1:
                mask = self.iri2_l1o # self.iri2_l1e if mir.ir.p == 1 else 
                mask[i:i+mir.dim] = True
            i += mir.dim
        ...
        alpha, x, num_ins = [], [], []
        self.instructions: list[Instruction] = [] # for compatibility with e3nn
            ...
            for ii2, mir_in2 in enumerate(self.iri2):
                for ii1, mir_in1 in enumerate(self.iri1):
                    if (mir_out.ir.l == 0 and (mir_in2.ir.l == mir_in1.ir.l)) or (mir_out.ir.l == 1 and (mir_in2.ir.l | mir_in1.ir.l)) \
                        ...
                        # alpha *= ins.path_weight # 1.0 for fully connected
        ...
        # alpha, x, num_ins = torch.tensor(alpha), torch.tensor(x), torch.tensor(num_ins)

        if self.is_comp_norm:
            # self.norm_l0 = torch.empty(dim_o_l0)
            # self.norm_l1 = torch.empty(self.dim_o_l1)
        ...
        #     totidx = 0
        #     for io, mir_out in enumerate(self.iro):
        #         x[totidx:num_ins[io]] *= num_ins[io]
        #         i2idx = 0
        #         for ii2, mir_in2 in enumerate(self.iri2):
        #             i2idx += mir_in2.
        #             for ii1, mir_in1 in enumerate(self.iri1):
        #                 if (mir_out.l == 0 and (mir_in2.l == mir_in1.l)) or (mir_out.l == 1 and (mir_in2.l | mir_in1.l)):
                            
        #         totidx += num_ins[io]

        #     if irrep_normalization == "component":
        #         alpha = torch.repeat_interleave(alpha, num_ins)
        #     elif path_normalization == "element":
        #         x = torch.repeat_interleave(x, num_ins)
        
        #     i_xg0 = x > 0.0
        #     alpha[i_xg0] /= x[i_xg0]

        #     self.norm_fact = torch.sqrt_(alpha)

        # build tp
        # with torch.no_grad():
        #     batchdim = 4
        #     dtype = self.weights_l0e.dtype
        #     example_inputs = (
        #         torch.zeros((batchdim, self.in1_dim), dtype=dtype),
        #         torch.zeros((batchdim, self.in2_dim), dtype=dtype),
        #         (self.weights_l0e, self.weights_l0o, self.weights_l1e, self.weights_l1o),
        #     )
        #     check_inputs = (example_inputs, (
        #         torch.zeros((batchdim+batchdim, self.in1_dim), dtype=dtype),
        #         torch.zeros((batchdim+batchdim, self.in2_dim), dtype=dtype),
        #         (self.weights_l0e, self.weights_l0o, self.weights_l1e, self.weights_l1o),
        #     ))
        #     self.regtraced_tp = torch.jit.trace(l1tp, example_inputs, check_inputs=check_inputs)
    ...
    def forward(self, in1: Tensor, in2: Tensor) -> Tensor:
        ...
        if self.dim_o_l0e > 0:
            ...
            # if self.dim_i1_l1e > 0 and self.dim_i2_l1e > 0: # doesnt exist (yet) as iri2 is sh
            #     o_l0e.append(self.cg110 * torch.linalg.vecdot(in1[:, self.iri1_l1e].reshape((-1, self.num_i1_l1e, 3)), in2[:, None, self.iri2_l1e]).reshape(-1, self.num_i1_l1e))
            ...
            # print("l1tp types (out, res, weights, in1)", out.dtype, res.dtype, self.weights_l0e.dtype, in1.dtype)
            del res#, o_l0e
            if self.is_comp_norm:
        ...
        if self.dim_o_l0o > 0: # more rare case
            ...
            # o_l0o.append(in1[:, self.iri1_l0e] * in2[:, self.iri2_l0o]) # doesnt exist (yet) as iri2 is sh
            ...
            # if (self.dim_i1_l1o > 0) and (self.dim_i2_l1e > 0):
            #     o_l0o.append(self.cg110 * torch.linalg.vecdot(in1[:, self.iri1_l1o].reshape((-1, self.num_i1_l1o, 3)), in2[:, None, self.iri2_l1e]).reshape(-1, self.num_i1_l1e))
    ...
    # def forward(self, in1: Tensor, in2: Tensor) -> Tensor:
    #     # self.iri1.dim doesnt work with torchscript
    #     torch._assert(in1.shape[-1] == self.in1_dim, "Incorrect last dimension for in1")
    #     torch._assert(in2.shape[-1] == self.in2_dim, "Incorrect last dimension for in2")
    #     out = empty((in1.shape[0], len(self.iro_l0e)), device=in1.device)
        
    #     # auto broadcast works only with sh in2
    #     o_l0e_l0 = in1[:, self.iri1_l0e] * in2[:, self.iri2_l0e]
    #     o_l0e_l1 = self.cg110 * torch.linalg.vecdot(in1[:, self.iri1_l1o].reshape((-1, self.num_i1_l1o, 3)), in2[:, None, self.iri2_l1o]).reshape(-1, self.num_i1_l1o)
    #     out[:, self.iro_l0e] = cat((o_l0e_l0, o_l0e_l1), -1) @ self.weights_l0e
    #     del o_l0e_l0, o_l0e_l1
    #     if self.is_comp_norm:
    #         out[:, self.iro_l0e] *= self.norm_l0e
    ...
    #     #     if self.dim_i1_l1e > 0:
    #     #         o_l0o.append(self.cg110 * torch.linalg.vecdot(in1[:, self.iri1_l1e].reshape((-1, self.num_i1_l1e, 3)), in2[:, None, self.iri2_l1o]).reshape(-1, self.num_i1_l1e))
    #     #     out[:, self.iro_l0o] = cat(o_l0o, -1) @ self.weights_l0o
    #     #     if self.is_comp_norm:
    #     #         out[:, self.iro_l0o] *= self.norm_l0o
    ...
    #     o_l1o_l01 = self.cg011 * in1[:, self.iri1_l0e, None] * in2[:, None, self.iri2_l1o]
    #     o_l1o_l10 = self.cg011 * in1[:, self.iri1_l1o].reshape(-1, self.num_i1_l1o, 3) * in2[:, None, self.iri2_l0e]
    #     # if self.dim_i1_l1e > 0:
    #     #     o_l1o.append(self.cg111 * torch.linalg.cross(in1[:, self.iri1_l1e].reshape(-1, self.num_i1_l1e, 3),  in2[:, None, self.iri2_l1o]))
    #     out[:, self.iro_l1o] = torch.tensordot(cat((o_l1o_l01, o_l1o_l10), -2), self.weights_l1o, ([-2], [0])).transpose(-1,-2).reshape(-1, self.dim_o_l1o) # why view doesnt work sometimes? .squeeze().repeat_interleave(3,-1)
    #     del o_l1o_l01, o_l1o_l10
    #     if self.is_comp_norm:
    #         out[:, self.iro_l1o] *= self.norm_l1o

    #     return out.contiguous()
    # ca. 16h work
    # # TODO einsum instead of reshape with special products?
    # def forward(self, in1: Tensor, in2: Tensor) -> Tensor:
    #     # self.iri1.dim doesnt work with torchscript
    #     torch._assert(in1.shape[-1] == self.in1_dim, "Incorrect last dimension for in1")
    #     torch._assert(in2.shape[-1] == self.in2_dim, "Incorrect last dimension for in2")
