...
def findlr(model: pl.LightningModule, trainer: pl.Trainer):
    ...
    lr_finder = tuner.lr_find(model, min_lr=5e-8, max_lr=0.8, num_training=1000, early_stop_threshold=None)
    ...
    # Plot with
    # fig = lr_finder.plot()
    # fig.savefig(f"lr_nb_{job_id}.png")
    loss = np.array(lr_finder.results["loss"])
    lr = np.array(lr_finder.results["lr"])
    idx_l10 = loss <= 10
    plt.plot(lr[idx_l10], loss[idx_l10], label="loss <= 10")
    plt.savefig(f"lr_nb_{job_id}.png")
...
if __name__ == "__main__":
    ...
    from nbody.train_gravity import HSEGNN, LitGravity
    ...
    model = LitGravity(HSEGNN, lr, 1e-12, 
                    input_irreps,
                    hidden_irreps,
                    output_irreps,
                    edge_attr_irreps,
    ...
    trainer = pl.Trainer(precision=prec, auto_lr_find=False, auto_scale_batch_size=False, enable_progress_bar=True, #profiler="simple", # log_every_n_steps=2, use_amp deterministic=True, 
        ...
        gradient_clip_val=100) #, StochasticWeightAveraging(swa_lrs=1e-2, swa_epoch_start=40)
